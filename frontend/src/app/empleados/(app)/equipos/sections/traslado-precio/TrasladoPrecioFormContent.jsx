...
const trasladoPrecioSchema = z.object({
  precio: z
    .number({
...
export default function TrasladoPrecioFormContent({ onFormSubmit }) {
  const { toast } = useToast()
  const { mutate } = useSWRConfig()

  const form = useForm({
    resolver: zodResolver(trasladoPrecioSchema),
    defaultValues: {
      precio: 0,
      fecha_desde: '',
    },
  ...
  const { trigger, isMutating } = useSWRMutation(
    ...
    storeFetcher,
    {
      onSuccess() {
        ...
        form.reset()
        mutate(key => Array.isArray(key) && key[0] === '/api/traslado-precios')
        onFormSubmit()
      },
      onError(err) {
