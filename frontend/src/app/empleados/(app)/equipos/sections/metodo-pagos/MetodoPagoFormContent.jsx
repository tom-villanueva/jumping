...
export default function MetodoPagoFormContent({
  onFormSubmit,
  metodo,
  editing,
}) {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      descuento_id: metodo?.descuento_id ? String(metodo?.descuento_id) : '',
    },
  })
  ...
  const { trigger, isMutating } = useSWRMutation(
    ...
    editing ? updateFetcher : storeFetcher,
    {
      onSuccess() {
        toast({
          title: editing
            ? `😄 Método modificado con éxito`
            : `😄 Método agregado con éxito`,
        })
        form.reset()
        mutate(key => Array.isArray(key) && key[0] === '/api/metodo-pagos')
        onFormSubmit()
      },
      onError(err) {
